...
- Release-time configuration and systemd invoke the built DSH CLI directly. The pnpm-backed source launcher remains limited to writable development checkouts and cannot reconcile dependencies inside an immutable release.
- Native Ollama listens only on the Docker bridge gateway on Linux. Harness and OpenViking remain loopback-only and are reached through an SSH tunnel; the MVP does not add a public reverse proxy.
