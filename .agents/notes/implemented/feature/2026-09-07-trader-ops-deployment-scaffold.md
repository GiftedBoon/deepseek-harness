...
- Native Ollama listens only on the Docker bridge gateway on Linux, and OpenViking remains loopback-only. Harness defaults to loopback and may bind one host-owned IPv4 address for trusted private-LAN access; the configurator rejects wildcard and non-local addresses, and the MVP does not add a public reverse proxy.
...
The Harness-side tool-name/environment boundary is active and explicitly denies OpenViking permanent-forget. The remaining unfinished area is the real Trader Ops MCP service and its trusted identity, resource/argument authorization, durable approval, and audit stores. A private-LAN listener relies on a bearer URL token rather than trusted user identity, so network access stays limited to trusted developers until that final service boundary exists.
