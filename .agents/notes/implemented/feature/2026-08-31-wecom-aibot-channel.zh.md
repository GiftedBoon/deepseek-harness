...
释放定时动作时还会清除全部计时器、中止并排空所有已启动工具调用，并把已经越过 running 提交点的调用留给结果不确定恢复。后台结果是主动企业微信通知和工具提供方审计记录；它们不会被伪造成 Session 历史中的 assistant 消息。
...
**把普通 Schedule 提醒变成命令。** 拒绝，因为 Schedule 会刻意把到期 prompt 作为未受信任的会话内容，不负责冷 Session 恢复，并会随按交付创建的企业微信 Agent 一起释放。把这种提醒当成权限来源还会让任意已保存文本变成远程操作。

**重启后重放每个 running 动作。** 拒绝，因为进程丢失本地结果前，已配置工具可能已提交非幂等远端副作用。报告结果不确定，可在这一故障窗口内保持至多一次调度。
