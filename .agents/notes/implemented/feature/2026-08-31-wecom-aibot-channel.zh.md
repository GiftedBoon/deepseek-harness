...
Wire 准入检查 BotID、文本类型、UTF-8 大小、发送者白名单与群聊白名单。原始提供方 id 仍不会出现在 DSH 标识符中。发送者被拒绝时，warning 只记录建立白名单所需的准确 JSON 引号 `userid`；已准入的提供方 id、消息文本与回调帧仍不会出现在日志中。插件拒绝带交互审批或 `danger-full-access` 的权限 preset。
