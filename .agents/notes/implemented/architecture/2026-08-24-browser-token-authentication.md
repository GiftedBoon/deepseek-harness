...
The [optional browser password login](../feature/2026-09-01-browser-password-login.md) reuses this cookie and complete-API authorization rule while adding another exchange path. This decision partially supersedes the authentication deferral and unauthenticated non-loopback consequences in the [browser trust note](2026-07-28-api-browser-trust-boundary.md). That note remains active authority for media-type, Host, Origin, Fetch-Metadata, and configured-authority validation. No active Agent Note is archived: the overlaps are partial and all three security rules retain future decision value.
