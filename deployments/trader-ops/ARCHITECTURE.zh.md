...
DeepSeek Harness (web profile)
  |-- Main LLM -----------> 通过 llm-pi-ai 连接选定的 OpenAI-compatible 路由
...
- 所有模型可见的提示词、检索记忆、工具描述和用户输入都会发送给 AIHubMix。在允许受限数据前，必须评审其端点、所选模型、保留策略和数据处理条款。
- 本地 Ollama 绑定 `127.0.0.1:11434`；Docker Desktop 通过 `host.docker.internal` 访问它的 embedding 和 query-planner 模型。
- 在 Debian MVP 上，原生 Ollama 只绑定 Docker bridge gateway。Compose 把 `host.docker.internal` 映射到该 gateway，Harness 与 OpenViking 则只为 SSH 隧道发布回环端口。
