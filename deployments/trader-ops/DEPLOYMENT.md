...
Build that archive with `git archive` alone: `git archive --format=tar.gz --prefix="$name/" --add-virtual-file="$name/.trader-ops-source-commit:$commit" -o "/tmp/$name.tar.gz" "$commit"`. Archiving through macOS `tar` instead writes one extended-attribute record per file, and GNU tar on the Debian host materializes each record as an `._*` AppleDouble file, which `sync-knowledge.mjs` rejects as a knowledge path that is not a lowercase ASCII slug.
...
The WeCom patch exposes two durable bssh_ops actions: `ps_check` runs the fixed read-only process check for one colo, while `quick_command_plan` accepts the exact `plan_id` returned by `prepare_quick_command`. Preparing the plan freezes its target, rendered command, and execution window before scheduling; the channel passes only `plan_id` to `execute_quick_command_plan` when due. Arbitrary shell commands are not exposed. A time-only request such as `15:01` resolves to the next occurrence at `+08:00`. The due notification includes the bssh_ops result; query a returned `run_id` through `get_run_status` when final remote output is needed. Generic Schedule remains reminder-only and does not execute these operations.
