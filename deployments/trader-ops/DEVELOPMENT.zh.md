...
- 企业微信 patch 通过 bssh_ops `run_quick_command` 暴露持久的 `ps_check`、`quick_command` 和 `custom_shell` 动作。目标表达式必须保持收窄；定时快捷命令前先获取当前 key 清单，持久化准确的自定义 shell 前必须完成语法检查并取得用户明确确认。
