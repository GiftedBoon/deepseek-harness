...
当你希望提醒作为消息出现在同一会话中时使用 Schedule——例如「30 分钟后提醒我跟进迁移」或「构建运行期间每小时提醒我查看一次」。Schedule 永远不会执行记录的工具或命令；未来副作用应使用部署提供的专用定时动作能力。agent 会通过它的普通工具为你创建、列出和取消提醒；你只需启用一次 overlay。
